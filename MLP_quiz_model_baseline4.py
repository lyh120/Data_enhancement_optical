...
def offline_training(num_epoch=500, Lip_lambda=0.2, L_const=np.ones([5,12]), save_path="enhanced_model.pth"):
    ...
    #加载主训练模型
    # loaded_data = np.load('augmented_100data_original.pth')
    # setting_list = loaded_data['inputs_original'].reshape(-1, 12)
    # q_list = loaded_data['outputs_original'].reshape(-1, 5)
    loaded_data = np.load('augmented_data_1000_V2.npz')
    setting_list = loaded_data['settings'].reshape(-1, 12)
    q_list = loaded_data['q_values'].reshape(-1, 5)
    ...
    #直接使用原始20组数据进行训练
    augmented_settings = test_data['settings'][:20].reshape(-1, 12)
    augmented_q = test_data['q_values'][:20].reshape(-1, 5)
...
if __name__ == "__main__":
    ...
    offline_training(num_epoch=300, Lip_lambda =500, L_const = Lip)
