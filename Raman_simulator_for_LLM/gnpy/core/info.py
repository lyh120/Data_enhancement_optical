...
def create_input_spectral_information_various(f_min, f_max, roll_off, baud_rate, power_arr, spacing, nli_arr=None, ase_arr=None):
    if nli_arr is None:
        # pref in dB : convert power lin into power in dB
        pref = lin2db(power_arr * 1e3)
        nb_channel = automatic_nch(f_min, f_max, spacing)
        si = SpectralInformation(
            pref=Pref(pref, pref, lin2db(nb_channel)),
            carriers=[
                Channel(f, (f_min + spacing * f),
                        baud_rate, roll_off, Power(power_arr[f-1], 0, 0), 0, 0) for f in range(1, nb_channel + 1)
            ]
        )
    else:
        # pref in dB : convert power lin into power in dB
        ...
        si = SpectralInformation(
            ...
            carriers=[
                Channel(f, (f_min + spacing * f),
                        baud_rate, roll_off, Power(power_arr[f-1], nli_arr[f-1], ase_arr[f-1]), 0, 0) for f in range(1, nb_channel + 1)
            ]
        )
    return si
