...
from numpy import interp, pi, zeros, shape, where, cos, reshape, array, append, ones, argsort, nan, exp, arange, sqrt, \
    empty, vstack, trapz, arcsinh, clip, abs, sum, zeros_like
...
def propagate_raman_fiber(fiber, *carriers):
    ...
    for carrier, attenuation, rmn_ase in zip(carriers, fiber_attenuation, raman_ase):
        if sim_params.nli_params.computed_channels == [0]:
            carrier_nli = nan
        else:
...
class RamanSolver:
    ...
    def calculate_spontaneous_raman_scattering(self, carriers, raman_pumps):
        ...
        if 'ns' in raman_efficiency:
            ns = raman_efficiency['ns']
        else:
            ns = 0
    ...
    def _int_spontaneous_raman(self, z_array, raman_matrix, alphap_fiber, freq_array,
                               cr_raman_matrix, freq_diff, ase_bc, bn_array, temperature, ns=0):
        ...
        for f_ind, f_ase in enumerate(freq_array):
            cr_raman = deepcopy(cr_raman_matrix[f_ind, :])

            freq_ref = 299792458.0 / (1550e-9)
            cr_raman *= (freq_array[f_ind]/freq_ref)**ns  # cf. Eq.(12) in [N.R.Newbury, JLT 21(12), pp.3364, Dec. 2003]
    ...
    def calculate_stimulated_raman_scattering(self, carriers, raman_pumps):
        ...
        def ode_function(z, p):
            return self._ode_stimulated_raman(z, p, alphap_fiber, freq_array, cr, prop_direct, ns=ns)
    ...
    def _ode_stimulated_raman(self, z, power_spectrum, alphap_fiber, freq_array, cr_raman_matrix, prop_direct, ns=0):
